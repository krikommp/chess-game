...
| MovementController 删除 AP 扣除，变为纯移动工具 | ⚠️ 已被 2026-05-19 二次修订取代：移动 AP 回到 Ability Costs 统一流程，由 ISkillUpdate 驱动 Cost Function 按实际距离扣 AP |
...
3. **Cost Function 是资源支付执行点**。普通技能在 `ApplyCosts` 阶段扣 AP；移动技能在 `ISkillUpdate` tick 中让 Cost Function 按实际位移扣 AP。`MovementController` 负责 NavMesh 移动执行、保留未支付移动距离预算和预算耗尽时停止移动。
...
basic_move:
  ...
  2. Execute 阶段（确认）:
     ...
     ASC.Update: GroundMoveAbility.SkillUpdate → SpendAPFunction.Update 根据实际累计移动距离扣 AP
```
...
- 2026-05-19 二次修订决议：**移动 AP 走 Ability Costs 统一流程**。`GroundMoveAbility` 显式调用 `ComputeCosts` / `ApplyCosts`，并实现 `ISkillUpdate`；ASC 持有 `SkillExecutionState`，在 `Update` 中调用 `GroundMoveAbility.SkillUpdate`；`SpendAPFunction.Update` 根据实际移动距离调用 `MovementController.TrySpendMoveDistance` 扣 AP。`MovementController` 不再在自身 `Update` 中主动扣 AP，只负责 NavMeshAgent 执行、未支付移动距离预算、预算耗尽时停止移动。影响：`MovementController.cs`、`GroundMoveAbility.cs`、`SkillAbility.cs`、`SkillEffectFunction.cs`、`SkillExecutionState.cs`、`ISkillUpdate.cs`、`SpendAPFunction.cs`、`CombatAssetSetupMenu.cs`、`Docs/05_SKILL_SPEC.md`。
- 2026-05-19 预览修订：**移动预览也走 Cost Function**。`SkillEffectFunction` 增加 `PreviewMaxPathLength`，`SkillAbility.PreviewCosts` 汇总所有 Cost 的路径限制；`GroundMoveAbility.ShowMovePreview` 和 `TryBuildMovePath` 都用该结果裁剪路径，不再直接以 `MovementController.RemainingMoveDistance` 作为预览来源。影响：`GroundMoveAbility.cs`、`SkillAbility.cs`、`SkillEffect.cs`、`SkillEffectFunction.cs`、`SpendAPFunction.cs`。
- 2026-05-19 修订决议（已被二次修订取代）：**MovementController 内建累计移动预算**。战局内所有 NavMesh 移动都按实际累计移动距离扣 AP：每累计走满 `MoveSpeed` 米扣 1 AP，未满 1 AP 的 `m_unpaidMoveDistance` 在本回合保留；`ResetMovementFunction` 在回合重置时调用 `ResetMovementBudget()` 清空。`GroundMoveAbility` 不再通过 `SpendAPEffect` 一次性预扣移动 AP，只负责输入解释、路径计算和 `MovementController.TryStartMove(path)`；`SpendAPFunction` 保留给攻击和普通技能固定 AP 消耗。影响：`MovementController.cs`、`GroundMoveAbility.cs`、`ResetMovementFunction.cs`、`CombatAssetSetupMenu.cs`、`Docs/02_SYSTEM_SPEC.md`、`Docs/05_SKILL_SPEC.md`。
- 来源：2026-05-12 技能框架重写
