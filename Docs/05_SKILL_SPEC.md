...
> **设计决议 (2026-05-12 / 2026-05-19 修订):** `SkillDefinition.apCost` 与 `SkillDefinition.cooldown` 不再作为设计字段保留。当前仍处设计阶段，资产依赖较少，直接删除旧字段，避免后续重构出现 `SkillDefinition` 字段与 Ability/Effect 槽位并存的双轨逻辑。攻击和普通技能 AP 消耗通过 Ability 的 `Costs` 槽位配置 `SpendAPEffect`；战局移动 AP 同样通过 `Costs` 槽位配置 `SpendAPEffect`，但移动上下文不在 `Apply` 阶段一次性预扣，而由 ASC 持有运行时执行状态并在 `ISkillUpdate` tick 中调用 Cost Function 更新，按实际累计移动距离扣除；冷却通过 Ability 的 `Cooldowns` 槽位配置 Persistent `SkillEffect` + `TagOnlyEffectFunction`，并使用 `Cooldown.{skillId}` Tag 表达。
...
- 主动移动：使用 `Costs` 检查与持续扣除移动 AP，不需要 `Effects`；移动 AP 的实际距离累计由 `GroundMoveAbility` 的运行时更新状态驱动 Cost Function，`MovementController` 只保留移动执行和未支付距离预算。
- 系统技能：可能只使用 `Effects`。
...
protected SkillCostPreviewResult PreviewCosts(SkillExecutionContext context, float maxPathLength);
...
`CostEffect` 层 Tag 条件用于资源支付规则，例如 `SpendAPEffect.BlockedTags = [State.APBlocked]` 表示技能 AP 支付被阻断，`State.FreeCast` 可用于免除或修改技能支付。战局移动 AP 也走 CostEffect 条件：`basic_move` 的 Costs 配置 `SpendAPEffect`，该 Function 在移动上下文中按实际累计距离持续扣除 AP。
...
- **Costs:** `SpendAP`（普通技能 Compute 检查/计算技能消耗，Apply 扣 AP；移动技能 Compute 只做启动前检查，Apply 不预扣，Update 按实际位移持续扣 AP）
...
移动预览必须和执行使用同一套 Cost 判断。`GroundMoveAbility` 在 hover 与点击确认时都构造 `SkillExecutionContext.ForGroundPoint(...)`，通过 `PreviewCosts(context, pathLength)` 查询 Cost Function 可允许的最大路径长度，再裁剪可达/不可达路径。不得在预览层直接用 `AP * MoveSpeed` 或 `MovementController.RemainingMoveDistance` 作为唯一判断，否则 `State.Rooted`、`State.APBlocked`、`State.FreeMove`、地形倍率等 Cost 规则会和执行分叉。
...
5. 若 AP 足够支持移动到可施放点，并足够支付目标技能的 `Costs`，则执行“移动技能 -> 等待移动完成 -> 重新校验 -> 攻击技能”。
...
- 技能不会消耗 AP 之外的其他资源。
- 攻击和普通技能 AP 消耗通过 Ability 的 `Costs` 槽位配置 `SpendAPEffect`，不再通过 `SkillDefinition.apCost` 字段表达；移动 AP 也通过 `GroundMoveAbility.Costs` 中的 `SpendAPEffect` 统一处理，扣费发生在 `ISkillUpdate` tick 的 Cost Function 更新阶段。
...
| `UnpaidMoveDistance` | 本回合已累计但尚未跨过 1 AP 阈值的移动距离 |
| `ResetMovementBudget()` | 回合切换时重置累计移动距离 |

事件：`MovementStarted`、`MovementStopped`、`ApDeducted`、`MoveBudgetDepleted`。
...
// 移动 AP 消耗由 GroundMoveAbility 的 ISkillUpdate 驱动 Cost Function，根据实际累计距离扣除
```
