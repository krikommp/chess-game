...
> **设计决议 (2026-05-12 / 2026-05-19 修订):** `SkillDefinition.apCost` 与 `SkillDefinition.cooldown` 不再作为设计字段保留。当前仍处设计阶段，资产依赖较少，直接删除旧字段，避免后续重构出现 `SkillDefinition` 字段与 Ability/Effect 槽位并存的双轨逻辑。攻击和普通技能 AP 消耗通过 Ability 的 `Costs` 槽位配置 `SpendAPEffect`；战局移动 AP 由 `MovementController` 按累计实际移动距离统一扣除；冷却通过 Ability 的 `Cooldowns` 槽位配置 Persistent `SkillEffect` + `TagOnlyEffectFunction`，并使用 `Cooldown.{skillId}` Tag 表达。
...
- 主动移动：不使用 `Costs` 扣移动 AP，不需要 `Effects`；移动 AP 由 `MovementController` 根据实际累计距离扣除。
- 系统技能：可能只使用 `Effects`。
...
`CostEffect` 层 Tag 条件用于资源支付规则，例如 `SpendAPEffect.BlockedTags = [State.APBlocked]` 表示技能 AP 支付被阻断，`State.FreeCast` 可用于免除或修改技能支付。战局移动 AP 是全局规则，由 `MovementController` 根据累计实际移动距离扣除。
...
- **Costs:** `SpendAP`（Compute 检查/计算技能消耗，Apply 扣 AP；移动 AP 不走 `SpendAP` 预扣）
...
- 技能不会消耗 AP 之外的其他资源。
- 攻击和普通技能 AP 消耗通过 Ability 的 `Costs` 槽位配置 `SpendAPEffect`，不再通过 `SkillDefinition.apCost` 字段表达；移动 AP 由 `MovementController` 按累计实际移动距离统一扣除。
...
// 移动 AP 消耗由 MovementController 在移动中根据实际累计距离扣除
```
