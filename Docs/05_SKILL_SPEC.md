...
> **设计决议 (2026-05-12):** `SkillDefinition.apCost` 与 `SkillDefinition.cooldown` 不再作为设计字段保留。当前仍处设计阶段，资产依赖较少，直接删除旧字段，避免后续重构出现 `SkillDefinition` 字段与 Ability/Effect 槽位并存的双轨逻辑。AP 消耗通过 Ability 的 `Costs` 槽位配置 `SpendAPEffect`；冷却通过 Ability 的 `Cooldowns` 槽位配置 `SetCooldownEffect`，并使用 Persistent Status + `Cooldown.{skillId}` Tag 表达。
...
- `AbilitySystemComponent`：通用入口，负责施法者存活、冷却、目标类型、目标能力、阵营、射程等通用校验。
...
所有可释放 `SkillDefinition` 必须显式配置 Ability。`basic_attack` 也应挂载类似 `SimpleTargetAbility` / `MeleeAttackAbility` 的显式 Ability，不允许 `Ability == null` 时由 `AbilitySystemComponent` 走隐藏默认流程。
...
`UnitTurnHandler` 负责玩家回合内的选择、结束回合和输入路由；它不解释具体技能输入。当前 MVP 规则是：选中角色后自动激活该角色自己的 `basic_move`；后续 UI 技能栏只需要改为激活其他 `SkillAbility`。`AbilitySystemComponent` 保存当前激活技能，并把输入请求写入 `SkillExecutionContext` 后转交给实现 `ISkillInputHandler` 的当前技能。

因此，输入层只表达“发生了什么输入”，玩家回合处理器只负责“当前谁接收输入”，技能或后续 Ability Targeting 层决定“这个输入对当前激活技能意味着什么”。

每个可行动角色必须在自身的 `AbilitySystemComponent.availableSkills` 中配置技能资产。没有配置技能的角色不能通过全局管理器获得默认行动；`CombatRoundManager` 只能报告缺失配置，不能自动补发技能。运行时生成的敌人必须由生成源（当前是 `EnemySpawner.m_defaultSkills`）把技能列表写入新生成对象的 `AbilitySystemComponent`。
...
public sealed class EffectDefinition : ScriptableObject
{
    ...
    [SerializeField] private ETargetCapability m_requiredCapability;
...
`EffectFunction` 示例：`SpendAP`、`SetCooldown`、`ModifyAttribute`、`AddStatus`、`ForcedMove`、`PullTarget`、`RestoreAttribute`、`ResetMovement`、`TriggerStatusTick`、`DestroyGameObject`。
...
| **Persistent** | `EEffectDuration.Persistent` | 注册到目标 `AbilitySystemComponent` 定时器，每回合 tick，过期自动 Remove | `BurningStatus`、`GuardingBuff` |
...
- **Cooldowns:** `SetCooldown`（Compute 检查冷却 Tag，Apply 添加冷却 Status/Tag）
...
- 玩家输入、敌方 AI、脚本化战术都应向对象自身的 `AbilitySystemComponent` 提交施放请求，而不是各自实现技能流程。
- 技能目标不应限定为敌人或玩家。木桶、墙壁、机关等可交互物体也可以成为技能目标。
- 不额外引入 `ISkillTarget` / `SkillTarget`。目标合法性统一由目标对象自身的 `AbilitySystemComponent` 判断。
- 只要对象挂载 `AbilitySystemComponent`，它就默认进入技能系统，可以被技能查询和影响。
- `AbilitySystemComponent` 应提供最小配置来声明该对象承受技能效果的能力，例如：
  - 是否可被伤害。
  - 是否可被治疗。
  - 是否可添加 Status。
  - 是否属于玩家、敌方、中立或环境目标。
  - 是否阻挡、可破坏或可互动。
- 第一阶段可以先让玩家和敌人的 `AbilitySystemComponent` 承担释放与受击能力；后续扩展木桶、墙壁等环境目标时，只需要给对象挂载并配置 `AbilitySystemComponent`。
...
CR-0002 Phase 2 之后，移动也属于技能流程：玩家点击地面本质上是当前激活的 `basic_move` 解释 `Input.Target.Ground` 输入请求。输入层只能发出 Tag 化输入请求，`UnitTurnHandler` 只能路由到当前单位的 `AbilitySystemComponent`，不能直接调用单位移动 API，也不能特判 `GroundMoveAbility` 后构造移动技能上下文。
...
1. 读取当前角色自身 `AbilitySystemComponent.availableSkills` 中配置的攻击技能，例如 `basic_attack`。
...
- 特定技能可以通过 Ability 的 `Cooldowns` 槽位配置 `SetCooldownEffect`，用于避免强力技能被连续使用。
- 冷却回合数不允许小数。
- 战斗中，`SetCooldownEffect(rounds=1)` 表示使用后需要等待 1 个回合单位的冷却。
- 探索 / 非战斗实时状态下，世界实时更新，技能冷却按秒刷新。
- 暂定 `1 回合 = 6 秒`，因此探索状态下 `SetCooldownEffect(rounds=1)` 等价于冷却 6 秒。
...
技能系统不应依赖任何角色类型（`Player1Controller`、`EnemyController`）。任何挂载 `AttributeSet` + `AbilitySystemComponent` 的 GameObject 都是合法的技能目标。
...
├── AbilitySystemComponent (MB)            ← 技能执行
