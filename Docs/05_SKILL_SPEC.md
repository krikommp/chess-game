...
`CombatRoundManager` 负责把输入请求分发给当前选中角色的 `SkillExecutor`。当前 MVP 规则是：选中角色后自动激活该角色自己的 `basic_move`；后续 UI 技能栏只需要改为激活其他 `SkillDefinition`。`SkillExecutor` 保存当前激活技能，并把输入请求写入 `SkillExecutionContext` 后调用该技能的 `SkillAbility.Execute(context)`。
